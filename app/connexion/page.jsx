...
export default function Connexion() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        setErrors({ ...nextErrors, email: data.error || 'Erreur lors de la connexion' });
      } else {
        setSubmitted(true);
        // Save the access token for simple client-side tracking if needed
        if (data?.data?.session?.access_token) {
           localStorage.setItem('supabase_token', data.data.session.access_token);
        }
  ...
  return (
    ...
    >
      <section className="w-full max-w-[1500px] lg:h-full min-h-[100vh] lg:min-h-0 lg:max-h-[calc(100vh-64px)] grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] xl:grid-cols-[minmax(0,1.75fr)_minmax(420px,0.9fr)] overflow-hidden rounded-none lg:rounded-[44px] bg-white shadow-none lg:shadow-[0_35px_90px_rgba(22,32,38,0.15)]" aria-label="Connexion professionnelle">
        <div 
        ...
        >
          ...
          <div className="relative z-10 mt-[30px] lg:mt-[clamp(50px,8vh,106px)]">
            <p className="m-0 uppercase tracking-[-0.045em] text-[clamp(2.4rem,11vw,4.2rem)] lg:text-[clamp(3rem,5.6vw,6.4rem)] font-[730] leading-[0.92]">CONNECTEZ-</p>
          ...
          <div className="absolute z-10 left-[28px] lg:left-[clamp(40px,5vw,78px)] right-[28px] lg:right-[clamp(40px,7vw,120px)] bottom-[30px] lg:bottom-[clamp(44px,6vw,76px)] grid grid-cols-[10px_1fr] items-stretch gap-[14px] lg:gap-[22px]">
            ...
            <p className="max-w-[760px] m-0 text-[0.9rem] lg:text-[clamp(1rem,1.35vw,1.35rem)] font-[650] leading-[1.5] lg:leading-[1.62] drop-shadow-md">
              Accédez à votre espace professionnel pour consulter l'état du réseau en temps réel, 
              suivre les alertes de coupures, et planifier efficacement vos interventions.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center p-[42px_26px_50px] lg:p-[clamp(42px,5vw,86px)_clamp(34px,4vw,72px)] bg-white/98 lg:overflow-y-auto">
          <div className="w-full max-w-[520px] mx-auto lg:my-auto">
            <div className="mb-[30px] lg:mb-[36px]">
              ...
              <h2 className="m-0 text-[#172026] text-[clamp(1.8rem,2.4vw,2.6rem)] leading-[1.1]">Connexion à votre espace</h2>
            </div>
            ...
            <form onSubmit={handleSubmit} noValidate>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-[24px] md:gap-y-[28px] md:gap-x-[20px]">
                ...
                <label className="block min-w-0 md:col-span-2">
                  <div className="flex justify-between items-end mb-[9px]">
                    <span className="block text-[#6f7477] text-[0.9rem] font-[750]">Mot de passe</span>
                    <a href="#" className="text-[0.8rem] font-[600] text-[#7c8285] hover:text-[#f58200] no-underline">Mot de passe oublié ?</a>
                  </div>
              ...
              <label className="flex items-start gap-[11px] my-[28px] text-[#676d70] text-[0.82rem] leading-[1.5] cursor-pointer">
                <input type="checkbox" className="w-[17px] h-[17px] flex-none mt-[2px] accent-[#f58200]" />
                <span>
              ...
              <p className="mt-[24px] mb-0 text-center text-[#7c8285] text-[0.85rem]">
                Pas encore de compte ? <a href="/inscription" className="text-[#db6c00] font-[800] no-underline hover:underline">Créer un compte</a>
              </p>
            </form>
          </div>
        </div>
